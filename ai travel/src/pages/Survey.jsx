            ...
            {step === 2 && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    {knowsDest === null ? (
                        ...
                        <section>
                            <label className="block text-sm font-medium text-gray-500 mb-3 uppercase tracking-wider">Where are you heading?</label>
                            <div className="relative">
                                <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 z-10" />
                                <input
                                    type="text"
                                    placeholder="Search destination (e.g. Seoul, Paris)..."
                                    className="w-full pl-12 pr-4 py-4 rounded-xl border-2 border-gray-100 focus:border-primary focus:outline-none text-lg font-medium transition-all"
                                ...
                                {formData.destination && (
                                    <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-gray-100 max-h-60 overflow-y-auto z-50">
                                        {[
                                            "South Korea (Seoul)", "South Korea (Busan)", "South Korea (Jeju)",
                                            "Japan (Tokyo)", "Japan (Kyoto)", "Japan (Osaka)", "Japan (Hokkaido)",
                                            "France (Paris)", "France (Nice)", "France (Lyon)",
                                            "Italy (Rome)", "Italy (Milan)", "Italy (Venice)", "Italy (Positano)",
                                            "USA (New York)", "USA (Los Angeles)", "USA (San Francisco)", "USA (Hawaii)",
                                            "UK (London)", "UK (Edinburgh)",
                                            "Spain (Barcelona)", "Spain (Madrid)",
                                            "Thailand (Bangkok)", "Thailand (Phuket)",
                                            "Vietnam (Da Nang)", "Vietnam (Hanoi)",
                                            "Indonesia (Bali)", "Australia (Sydney)"
                                        ].filter(city => city.toLowerCase().includes(formData.destination.toLowerCase()) && city !== formData.destination).map((city) => (
                                            <button
                                                key={city}
                                                onClick={() => updateData('destination', city)}
                                                className="w-full text-left px-6 py-3 hover:bg-gray-50 text-gray-700 font-medium transition-colors flex items-center gap-2"
                                            >
